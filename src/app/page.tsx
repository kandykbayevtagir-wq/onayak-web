...
export default function Home() {
  ...
  const [isDeliveryModalOpen, setIsDeliveryModalOpen] = useState(false); // Модалка доставки
  const [selectedProduct, setSelectedProduct] = useState(""); // Выбранный товар
  ...
  // УНИВЕРСАЛЬНЫЙ ОБРАБОТЧИК (Для записи и для магазина)
  const handleSubmit = async (e: React.FormEvent, type: 'appointment' | 'delivery') => {
    e.preventDefault();
    setIsSubmitting(true);
    try {
      // Для доставки нам не нужна дата, но нужен товар
      const dbPayload: any = {
      ...
      if (type === 'appointment') {
        ...
        dbPayload.appointment_time = formData.date;
      } else {
        dbPayload.problem = selectedProduct; // В колонку problem пишем название товара
      }
      ...
      setTimeout(() => { 
        ...
        setIsSuccess(false); 
        setFormData({ name: "", problem: "", date: "", comment: "" }); 
        setSelectedProduct("");
      }, 3000);
      ...
      <main className={`min-h-screen flex items-center justify-center p-6 transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-gray-100 text-gray-900'}`}>
        <div className={`border p-8 rounded-3xl w-full max-w-sm text-center shadow-2xl transition-colors ${theme === 'dark' ? 'bg-[#111] border-white/10' : 'bg-white border-gray-200'}`}>
          ...
          <div className="flex flex-col gap-3">
            <button onClick={() => {setLang("kz"); localStorage.setItem('onayak_lang', "kz");}} className={`w-full py-4 rounded-xl font-bold border ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/5' : 'bg-gray-50 border-gray-200'}`}>Қазақ тілі</button>
            <button onClick={() => {setLang("ru"); localStorage.setItem('onayak_lang', "ru");}} className="w-full py-4 bg-blue-600 rounded-xl text-white font-bold">Русский язык</button>
          </div>
        </div>
      </main>
    );
    ...
    return (
      <main className={`min-h-screen flex items-center justify-center p-6 transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-gray-100 text-gray-900'}`}>
        <div className={`border p-8 rounded-3xl w-full max-w-sm text-center shadow-2xl transition-colors ${theme === 'dark' ? 'bg-[#111] border-white/10' : 'bg-white border-gray-200'}`}>
          ...
          <button onClick={async () => { setHasAcceptedTerms(true); localStorage.setItem('onayak_terms', 'true'); if(tgUser?.id) await supabase.from('profiles').update({ terms_accepted: true, lang: lang }).eq('tg_id', tgUser.id); }} className="w-full py-4 bg-blue-600 rounded-xl text-white font-bold"><CheckCircle2 size={18} className="inline" /> {t.acceptTermsBtn}</button>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className={`min-h-screen flex flex-col font-sans transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      {/* --- НОВАЯ ВКЛАДКА: МАГАЗИН (ДОСТАВКА) --- */}
      {activeTab === "shop" && (
        <div className="p-5 flex-1 flex flex-col">
          <div className="flex items-center gap-2 mb-4">
            <ShoppingBag className="text-pink-500" size={24}/>
            <h2 className="text-xl font-black">{t.shopTitle}</h2>
          </div>
              ...
              <div key={prod.id} className={`p-5 rounded-3xl border flex flex-col gap-3 ${theme === 'dark' ? 'bg-[#111] border-white/10' : 'bg-white border-gray-200'}`}>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[10px] opacity-60 leading-relaxed">{prod.desc}</p>
                  </div>
                  <div className={`p-2 rounded-xl ${theme === 'dark' ? 'bg-pink-500/20' : 'bg-pink-50'} text-pink-500`}><Package size={20}/></div>
                </div>
                <button onClick={() => { setSelectedProduct(prod.name); setIsDeliveryModalOpen(true); }} className={`w-full py-3 mt-2 rounded-xl text-xs font-bold transition-all ${theme === 'dark' ? 'bg-white/10 hover:bg-white/20' : 'bg-gray-100 hover:bg-gray-200'}`}>
                  {t.orderBtn}
                </button>
              </div>
            ))}
            ...
            <div className="flex flex-col gap-3">
              {leads.map(lead => (
                <div key={lead.id} className={`p-4 rounded-2xl border relative overflow-hidden ${theme === 'dark' ? 'bg-[#111] border-white/5' : 'bg-white border-gray-200'}`}>
                  ...
                  {/* Иконка типа (Запись или Магазин) */}
                  <div className="absolute top-4 right-4 opacity-20">
                    {lead.lead_type === 'delivery' ? <Package size={40}/> : <Calendar size={40}/>}
                  </div>

                  <div className={`inline-block text-[8px] font-bold px-2 py-1 rounded-md mb-2 uppercase ${(!lead.status || lead.status === 'new') ? 'bg-yellow-500/20 text-yellow-500' : lead.status === 'in_progress' ? 'bg-blue-500/20 text-blue-500' : 'bg-green-500/20 text-green-500'}`}>
          ...
          {leads.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {leads.map(lead => {
                ...
                const isDelivery = lead.lead_type === 'delivery'; // Проверка типа
                  ...
                  <div key={lead.id} className={`p-4 rounded-2xl border relative overflow-hidden ${theme === 'dark' ? 'bg-[#111] border-white/5' : 'bg-white border-gray-200'} ${isCompleted ? 'opacity-50' : ''}`}>
                    ...
                    <div className="flex justify-between items-start mb-2 mt-1">
                      <div className="flex items-center gap-2">
                        {/* Иконка типа заявки в CRM */}
                        {isDelivery ? <Package size={16} className="text-pink-500"/> : <Calendar size={16} className="text-blue-500"/>}
                    ...
                    {/* Перенос времени только для приемов */}
          ...
          <div className={`border rounded-3xl w-full max-w-md p-6 relative shadow-2xl animate-in fade-in zoom-in-95 ${theme === 'dark' ? 'bg-[#111] border-white/10' : 'bg-white border-gray-200'}`}>
            ...
            {isSuccess ? (
              ...
              <>
                ...
                <form onSubmit={(e) => handleSubmit(e, 'appointment')} className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {t.problems.map((prob, idx) => (<button key={idx} type="button" onClick={() => setFormData({...formData, problem: prob})} className={`text-[10px] font-bold py-2 px-3 rounded-lg border transition-all ${formData.problem === prob ? "bg-blue-600 border-blue-600 text-white shadow-md" : "opacity-40"}`}>{prob}</button>))}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto pt-20 pb-10">
          <div className={`border rounded-3xl w-full max-w-md p-6 relative shadow-2xl animate-in fade-in zoom-in-95 ${theme === 'dark' ? 'bg-[#111] border-pink-500/20' : 'bg-white border-pink-200'}`}>
            ...
            {isSuccess ? (
              ...
              <>
                <div className="w-12 h-12 bg-pink-500/20 text-pink-500 rounded-xl flex items-center justify-center mb-4"><Package size={24}/></div>
                <h3 className="text-xl font-black mb-1">{t.deliveryModalTitle}</h3>
                <p className="text-xs opacity-60 mb-6">Администратор уточнит наличие и свяжется с вами.</p>
                <form onSubmit={(e) => handleSubmit(e, 'delivery')} className="flex flex-col gap-4">
                  <div><label className="block text-[10px] font-bold uppercase opacity-50 mb-1">{t.nameLabel}</label><input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className={`w-full border rounded-xl px-4 py-3 text-sm outline-none ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/10 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'}`} /></div>
                  
                  {/* Выбор товара (подхватывается из карточки, но можно изменить) */}
                  <div>
                    <label className="block text-[10px] font-bold uppercase opacity-50 mb-1">{t.productLabel}</label>
                    <div className="flex flex-wrap gap-2">
                      {t.products.map((prob) => (<button key={prob.id} type="button" onClick={() => setSelectedProduct(prob.name)} className={`text-[10px] font-bold py-2 px-3 rounded-lg border transition-all ${selectedProduct === prob.name ? "bg-pink-600 border-pink-600 text-white shadow-md" : "opacity-40"}`}>{prob.name}</button>))}
                    </div>
                  </div>

                  <div><label className="block text-[10px] font-bold uppercase opacity-50 mb-1">Уточнения (какой именно крем нужен и т.д.):</label><textarea rows={2} value={formData.comment} onChange={(e) => setFormData({...formData, comment: e.target.value})} className={`w-full border rounded-xl px-4 py-3 text-sm outline-none resize-none ${theme === 'dark' ? 'bg-[#1a1a1a] border-white/10 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'}`} placeholder="Например: мазь от пота для мужа..." /></div>
