...
import { MapPin, Star, ShieldCheck, Instagram, Menu, X, UserCog, Mail, Info, CalendarPlus, Database, Globe, CheckCircle2, ChevronRight, Activity } from "lucide-react";
// @ts-ignore
...
const DICT = {
  ru: {
    ...
    aboutHeadline: "Цифровая Экосистема",
    aboutText: "OnAyak — это инновационная B2B платформа для автоматизации и масштабирования центров профессиональной подологии по всему Казахстану."
  },
  kz: {
    ...
    aboutHeadline: "Цифрлық Экожүйе",
    aboutText: "OnAyak — бұл Қазақстан бойынша кәсіби подология орталықтарын автоматтандыруға және дамытуға арналған инновациялық B2B платформасы."
  }
};
...
export default function Home() {
  ...
  const [isAboutOpen, setIsAboutOpen] = useState(false); // Состояние для окна "О приложении"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Автоматически берем Telegram юзернейм или ID, чтобы не просить номер телефона
      ...
      const { error } = await supabase
        ...
        .insert([{ 
          ...
          client_phone: tgContact, // База ждет телефон, мы хитро передаем контакт ТГ
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white flex flex-col font-sans relative overflow-x-hidden">
      ...
      {/* Боковое меню */}
      <div className={`fixed inset-y-0 left-0 w-[80%] max-w-[300px] bg-[#111] border-r border-white/10 z-50 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? "translate-x-0" : "-translate-x-full"} flex flex-col`}>
        ...
        <div className="p-5 flex flex-col gap-6 flex-1 overflow-y-auto custom-scrollbar">
          {/* Смена языка */}
          <div>
            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-3">{t.langTitle}</p>
            <div className="flex bg-[#1a1a1a] rounded-lg p-1 border border-white/5">
              <button 
                onClick={() => setLang("ru")} 
                className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${lang === "ru" ? "bg-blue-600 text-white shadow-md" : "text-gray-400 hover:text-white"}`}
              >
                Русский
              </button>
              <button 
                onClick={() => setLang("kz")} 
                className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${lang === "kz" ? "bg-blue-600 text-white shadow-md" : "text-gray-400 hover:text-white"}`}
              >
                Қазақша
              </button>
            </div>
          ...
          <div>
            ...
            <button 
              ...
              className="w-full flex items-center gap-3 text-sm text-gray-300 py-2 hover:text-white transition-colors"
            >
      ...
      {/* АНИМИРОВАННОЕ ОКНО "О ПРИЛОЖЕНИИ" */}
      {isAboutOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => setIsAboutOpen(false)}></div>
          <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl w-full max-w-sm p-8 relative shadow-[0_0_50px_rgba(59,130,246,0.2)] animate-in zoom-in duration-300 overflow-hidden">
            {/* Декоративные летящие элементы фона */}
            <div className="absolute -top-20 -right-20 w-40 h-40 bg-blue-500/20 blur-[50px] rounded-full"></div>
            <div className="absolute -bottom-20 -left-20 w-40 h-40 bg-purple-500/20 blur-[50px] rounded-full"></div>

            <button onClick={() => setIsAboutOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white z-10">
              <X size={24} />
            </button>

            <div className="relative z-10 text-center flex flex-col items-center">
              <div className="w-20 h-20 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl flex items-center justify-center mb-6 animate-float animate-glow shadow-2xl border border-white/20">
                <Activity size={36} className="text-white" />
              </div>
              <h2 className="text-3xl font-black mb-2 gradient-text tracking-tighter">OnAyak</h2>
              <h3 className="text-sm font-bold text-gray-300 uppercase tracking-widest mb-6">{t.aboutHeadline}</h3>
              <p className="text-sm text-gray-400 leading-relaxed mb-8">
                {t.aboutText}
              </p>
              
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 w-full flex items-center justify-between">
                <div className="text-left">
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest">Версия / Version</p>
                  <p className="text-sm font-bold text-white">v 1.2.0 (B2B Core)</p>
                </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#111] border border-white/10 rounded-3xl w-full max-w-md p-6 relative shadow-2xl animate-in fade-in zoom-in duration-200">
            <button 
              onClick={() => setIsModalOpen(false)} 
              className="absolute top-5 right-5 text-gray-400 hover:text-white bg-white/5 rounded-full p-1"
            >
              <X size={20} />
            </button>
            ...
            {isSuccess ? (
              ...
              <>
                <h3 className="text-xl font-black text-white mb-6 pr-8">{t.modalTitle}</h3>
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  <div>
                    <label className="block text-xs text-gray-400 mb-2 uppercase tracking-wider font-bold">{t.nameLabel}</label>
                    <input 
                      type="text" 
                      required
                      value={formData.name}
                      onChange={(e) => setFormData({...formData, name: e.target.value})}
                      className="w-full bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3.5 text-sm text-white outline-none focus:border-blue-500 transition-colors"
                      placeholder={lang === "kz" ? "Атыңыз" : "Иван Иванов"}
                    />
                  </div>
                  
                  <div>
                    <label className="block text-xs text-gray-400 mb-2 uppercase tracking-wider font-bold">{t.problemLabel}</label>
                      ...
                      {t.problems.map((prob, idx) => (
                        <button
                          ...
                          className={`text-xs font-bold py-2 px-3 rounded-lg border transition-all ${
                            ...
                            : "bg-[#1a1a1a] border-white/5 text-gray-400 hover:border-white/20 hover:text-white"
                          }`}
                        >
                          {prob}
                        </button>
